const projects = [
  {
    title: "Ecommerce Website",
    img: "https://media.istockphoto.com/id/1397445959/vector/3d-vector-render-online-shopping-bag-for-product-add-to-card-on-website-cartoon-icon.jpg?s=612x612&w=0&k=20&c=rYebgSm-rFSD7_BcYDOHcRa2SUrdJdZKZpUHiAxKiEI=",
    gh: "https://github.com/Mayanksaharkar/E-commerce-webapp",
    description: "MERN | TAILWINDCSS ",
  },
  {
    title: "LifeTrack App",
    img: "https://res.cloudinary.com/defcox81s/image/upload/v1747306700/lifetrack_nvmxcg.jpg",
    gh: "https://github.com/Mayanksaharkar/lifeTrack",
    description: "React Native | Expo | Firebase",
  },
  {
    title: "Legal Assistant",
    img: "https://res.cloudinary.com/defcox81s/image/upload/v1747044709/legalAssistant.jpg",
    gh: "https://github.com/Mayanksaharkar/LegalAssitant",
    description: "MERN | Gemini API | TailwindCSS",
  },
  {
    title: "Time Table Generator",
    img: "https://res.cloudinary.com/defcox81s/image/upload/v1747049928/timeTable.jpg",
    gh: "https://github.com/Mayanksaharkar/Time_Table_Generator",
    description: "Andriod Studio | JAVA",
  },
  {
    title: "Web Scraping",
    img: "https://res.cloudinary.com/defcox81s/image/upload/v1747045405/webScraping_z7g323.webp",
    gh: "https://github.com/Mayanksaharkar/WebScraping",
    description: "Python | BeautifulSoup | Requests",
  },
  {
    title: "OCR-CLI Tool",
    img: "https://res.cloudinary.com/defcox81s/image/upload/v1747308536/ocr_jhuadi.png",
    gh: "https://github.com/Mayanksaharkar/ocr_cli",
    description: "Python | Tesseract",
  },
  {
    title: "Link Shortner",
    img: "https://res.cloudinary.com/defcox81s/image/upload/v1747044708/linkShortner.jpg",
    gh: "https://github.com/Mayanksaharkar/Link_Shortner",
    description: "ReactJS | TailwindCSS | DaisyUI",
  },

  {
    title: "Calculator App",
    img: "https://res.cloudinary.com/defcox81s/image/upload/v1747046706/calculator.jpg",
    gh: "https://github.com/Mayanksaharkar/Calculator",
    description: "ReactJS | DaisyUI ",
  },
];

export default projects;
